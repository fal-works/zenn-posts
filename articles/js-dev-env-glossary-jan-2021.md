...
というのも、[ESモジュール](#ESモジュール)が登場する前は他の[モジュールシステム](#モジュールシステム)しか選択肢が無かったため、Node.js で実行するつもりが無くても、利用者が後でうまいことバンドルする前提で CommonJS 形式（多くの場合はそれ以外も兼ねた [UMD](#umd)）によるエクスポートを行っていたのである。
