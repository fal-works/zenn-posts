...
JavaScript による開発に向けて道具や環境を整えるにあたり、実に様々な用語や概念があります。
...
あるいは、その言語で書かれたコードをチェックしたり JavaScript に[トランスパイル](#トランスパイル)したりする機能を提供する公式のパッケージ（[npm](#npm) でインストール可能）。
...
ソースコードなどのバージョン管理システム。変更履歴を記録する。
...
~~怪しい設計図共有サイト~~
ソースコードなどを保存し、広く公開したり、多人数で開発したりできるサービス。
使うには [Git](#git) が必要。
...
ブラウザーには数種類あり、[EcmaScript](#ecmascript) への対応の進み方が一律ではない。もし、自分の書いたコードができるだけ多くのブラウザーで動作することを望むなら、その違いを意識する必要がある。
...
……以上からすると、ブラウザーで動かすコードを書きたいだけの人からは一見して無縁に見えるかもしれない。しかし、Node.js 自体に含まれている [npm](#npm) を始め、各種有用なツールを動作させるための実行環境として Node.js が必要なため、実際は JavaScript を書く人の多くがお世話になる。
...
実際に JavaScript のコードを解釈して動かすのは各種実行環境（[ブラウザー](#Webブラウザー)や [Node.js](#node.js)）なので、どの環境でどの版までが有効なのかは一律ではない。「Can I use...」（<https://caniuse.com/>）というサイトで、個別機能ごとのブラウザ対応状況を確認できる。
...
- 比較的新しいバージョンの [EcmaScript](#ecmascript) に則って書かれた JavaScript のソースコードを、安定動作のため、より古いバージョンの EcmaScript の仕様に合うように変換する（[ES5](#es5) も参照）。
...
結果、それらのパッケージがエクスポートしている[モジュール](#モジュール)を自分のソースコードの中でインポートしたりできる。
...
pnpm は、見かけ上は従来のパッケージマネージャーと同じように動きつつ、インストールしたパッケージ群の実体は一箇所で一元管理することで、無駄なダウロードやファイルコピーを極力行わない。ついでに、依存関係が複雑な場合に整合性が取りにくくなる問題も解決する（らしい）。
...
細かくルールを設定できるが、`recommended` なルール郡を一括適用して手軽に始めることもできる。
参考：「ESLintのルールを全部手動で設定するのは大変だからやめておけ」
<https://qiita.com/khsk/items/0f200fc3a4a3542efa90>

ESLint に怒られながらコードを書いていく過程で、推奨されるコードの書き方が自然に見につく……かもしれない。
...
もともと ESLint にはバグ予防だけでなく[フォーマッター](#コードフォーマッター)としてのコード整形機能も入っているのだが、コード整形についてはそれを専門とする Prettier に全面的に任せる方法が主流となりつつある。
...
少なくとも開発段階においては、でかいファイルに長々とコードを書くよりも、機能ごとに小さく部品化してそれらを組み合わせるような作り方のほうが、作りやすいし第三者から見ても分かりやすい。
...
- ある JavaScript/TypeScript モジュールの中で、他のモジュールから使ってもらうことを意図して何らかの機能要素（変数、関数、クラスなど）を公開することを「エクスポート」と言う。
...
また最近のブラウザでは、ESモジュールのファイルを[バンドル](#バンドル)せずにブラウザ上でそのまま動かすこともできる。
参考：「JavaScript モジュール - JavaScript | MDN」
<https://developer.mozilla.org/ja/docs/Web/JavaScript/Guide/Modules>

## CommonJS

ブラウザ以外の環境における JavaScript の仕様について策定したもの。また、そのプロジェクト。
もともとブラウザで動かすための言語として発達した JavaScript だが、サーバーとかコンソールとか、ブラウザ以外の環境でも使えるようになったほうが嬉しい、みたいな動機である。
...
[モジュールシステム](#モジュールシステム)の文脈で IIFE と言うときは、[モジュール](#モジュール)全体を IIFE で包んで、インポートやエクスポートに相当することをやりたいときはグローバル変数を介して行う、という方式のことを指す。
...
複数の[モジュール](#モジュール)ファイルについてインポート／エクスポートの関係を読み取り、依存関係にあるモジュール群の全体を一つのファイルにまとめあげること。主にブラウザー環境で動かすために行う。

背景として、

- かつてはブラウザーで[ESモジュール](#esモジュール)を使えず、今でも全てのブラウザーで使えるわけではない
- 使えたとしても、`<script>`タグをいくつも書いてロードするのやだなあと思うこともある
...
JavaScript のファイルだけでなく、CSS やら画像やらも含めていろいろやってくれる。名前に Web と付いている通り、WebサイトやWebサービスをまるごと作るときなどの用途で広く使われる。
...
-  ライブラリに限らず小規模なパッケージや、バンドル対象ファイルが `*.js`/`*.ts` だけのパッケージなどに向いていると思われる。

ただし [TypeScript](#typescript) ファイルのバンドルはデフォルトでは行えず[プラグイン](#プラグイン)が必要になるのだが、いくつかあってどれが良いのか分かりにくかったり、微妙に動作が安定しなかったりするのが惜しい。
...
- テストフレームワーク
- Babel（トランスパイラー）
- WebGL（GPU使ってブラウザでグラフィックス高速描画する技術）
- Electron（JS等のWeb技術でデスクトップアプリを作れるフレームワーク）
- React（なんかすごいフレームワーク）
- WebAssembly/WASM（ブラウザ上で高速動作する低水準プログラミング言語）
